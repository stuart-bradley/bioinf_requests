class Emailer < ActionMailer::Base
  ...
  def new_request(id)
    ...
    employee = User.find_by login: @request.name
    if employee != nil
  ...
  def edit_request(id)
    ...
    if @request.check_for_edits_email == false
      ...
      emails = []
      users = User.where(login: @request.get_users)
      users.each do |u|
        emails << u.email
      end
  ...
  def edit_assignment(id)
    ...
    if edit_type_assignment.length > 0
      if employee != nil
        ...
        mail :to => [employee.email, emails], :from => "SynBioAdmin@lanzatech.onmicrosoft.com", :subject => "Request: '#{@request.title}' has been assigned", template_name: 'edit_assignment'
      end 
    end
  end
